...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    elseif (isset($_POST["admin_login"])) {
        ...
        if (mysqli_stmt_prepare($stmt, $sql)) {
            ...
            } else if ($admin && $password === $admin["password"]) { // Plain text password comparison
    ...
    elseif (isset($_POST["register"])) {
        ...
        if ($password !== $password_repeat) {
            ...
            $password_plain = $password; // Store plain text password (not recommended)

            $sql = "INSERT INTO users (full_name, email, password) VALUES (?, ?, ?)";
            $stmt = mysqli_stmt_init($conn);
            if (mysqli_stmt_prepare($stmt, $sql)) {
                mysqli_stmt_bind_param($stmt, "sss", $fullname, $email, $password_plain);
